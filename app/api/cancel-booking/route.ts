...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (fetchError || !booking) {
      console.error('Error fetching booking by cancellation token:', fetchError);
      return new NextResponse(`
        <!DOCTYPE html>
        <html>
        <head><title>Error de Cancelación</title></head>
    ...
    if (booking.google_calendar_event_id) {
      try {
        const googleCalendarResponse = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/google-calendar`, {
          ...
          body: JSON.stringify({ eventId: booking.google_calendar_event_id }),
        });

        if (!googleCalendarResponse.ok) {
          const errorText = await googleCalendarResponse.text();
          console.error(`Error al eliminar evento de Google Calendar ${booking.google_calendar_event_id}:`, errorText);
        } else {
          console.log(`Evento de Google Calendar ${booking.google_calendar_event_id} eliminado.`);
        }
      } catch (calendarError: any) {
        console.error('Error inesperado al eliminar evento de Google Calendar:', calendarError.message);
      }
    }
    ...
    try {
        const emailResponse = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/send-email`, {
            ...
            body: JSON.stringify({
                ...
                isCancellationConfirmation: true, // Indica que es una confirmación de cancelación
            }),
        });

        if (!emailResponse.ok) {
            const errorText = await emailResponse.text();
            console.error("Error al enviar email de confirmación de cancelación:", errorText);
        } else {
            console.log("Email de confirmación de cancelación enviado.");
        }
    } catch (emailError: any) {
        console.error("Error inesperado durante el envío del email de confirmación de cancelación:", emailError.message);
    }
