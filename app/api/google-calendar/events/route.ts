...
if (!GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET || !GOOGLE_REDIRECT_URI) {
  console.error(
    "Error: Faltan variables de entorno para Google OAuth en el servidor. Revisar .env.local"
  );
...
async function getAuthenticatedCalendarClient(
  ...
  if (
    ...
    throw new Error(
      "Configuración de Google Calendar no encontrada o incompleta para este Owner. Por favor, asegúrate de que el Owner haya conectado su calendario."
    );
  }
...
export async function POST(req: Request): Promise<NextResponse> {
  try {
    const {
      ...
      endDateTime,
      timezone,
    }: CreateEventRequest = await req.json();
    ...
    if (
      ...
      return NextResponse.json(
        {
          message:
            "Faltan detalles esenciales del evento o la clave del Owner.",
        },
        { status: 400 }
      );
    }
    ...
    const { calendar, calendarId } =
      await getAuthenticatedCalendarClient(ownerSecretKey);
    console.log(`timezone tenant:${timezone}`);
    const event: calendar_v3.Schema$Event = {
      summary: summary,
      description: description,
      start: { dateTime: startDateTime, timeZone: timezone },
      end: { dateTime: endDateTime, timeZone: timezone }, // Ajusta la zona horaria según sea necesario
      reminders: {
        useDefault: false,
        overrides: [
    ...
    const response: { data: { id?: string | null } } =
    ...
    return NextResponse.json(
      { message: "Evento creado exitosamente", eventId: eventId },
      { status: 200 }
    );
  } catch (error: any) {
    ...
    return NextResponse.json(
      {
        error: "Fallo al crear evento de Google Calendar",
  ...
  try {
    ...
    if (!ownerSecretKey || !eventId) {
      return NextResponse.json(
        {
          message:
            "Faltan detalles esenciales para eliminar o la clave del Owner.",
        },
        { status: 400 }
      );
    }
    ...
    console.log(
      `Evento de Google Calendar eliminado: ${eventId} para Owner: ${ownerSecretKey}`
    );
    return NextResponse.json(
    ...
    return NextResponse.json(
      {
        error: "Fallo al eliminar evento de Google Calendar",
        details: error.message || "Error desconocido",
      },
      { status: (error as any).code || 500 }
    );
